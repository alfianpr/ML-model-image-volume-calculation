...
class App(tk.Tk):
    def __init__(self):
        super().__init__()
        global threshold, open_morphology
    ...
    def create_widgets(self):
        ...
        # Button telur
        btn_telur = ttk.Button(self, text='Telur', command=self.submit)
        btn_telur.grid(column=0, row=0, **padding)

        # Entry
        name_entry = ttk.Entry(self, textvariable=self.jarak_var)
        name_entry.grid(column=1, row=0, **padding)
        name_entry.focus()

        # Button beras
        btn_beras = ttk.Button(self, text='Beras', command=self.submit)
        btn_beras.grid(column=2, row=0, **padding)

        # Output jumlah pixel
        self.lbl_pixel = ttk.Label(self)
        self.lbl_pixel.grid(column=0, row=1, columnspan=3, **padding)
        ...
        self.lbl_pixel.grid(column=1, row=2, **padding)

        # Output predict
        self.lbl_predict = ttk.Label(self, textvariable=self.predict)
        self.lbl_predict.grid(column=1, row=2, **padding)
    ...
    def submit(self):
        ...
        # predict
        predict = self.predict(self.jarak_var, jumlah_pixel)
